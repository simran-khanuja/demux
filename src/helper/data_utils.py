...
def load_train_datasets(
    ...
    if args.source_dataset_path is not None:
        ...
        for source_language in source_languages:
            ...
            # Add a language column to the dataset
            with accelerator.main_process_first():
    ...
    # Add en to train datasets
    en_raw_train_dataset=None
    en_processed_train_dataset=None
    if args.dataset_name == "tydiqa":
        en_raw_train_dataset = get_tydiqa_dataset(language="en", split="train", cache_dir=args.cache_dir)
        with accelerator.main_process_first():
            logger.info("Adding language to EN train dataset")
            en_raw_train_dataset = en_raw_train_dataset.map(partial(add_language, language="en"), batched=True)

            en_processed_train_dataset= en_raw_train_dataset.map(
            ...
            logger.info(f"Processed train dataset size for en: {len(en_processed_train_dataset)}")
